...
const ModalSettings = ({
    ...
    return (
        <>
            ...
            <div className={`${openSetting ? '' : 'hidden'} max-w-xl text-[#111827] absolute bg-[#F1FAEE] sm:w-96 w-11/12 left-1/2 top-1/2 rounded-md p-5`} style={{
                ...
                <div className='h-1 w-full bg-[#111827] my-5'></div>
                <div className='flex gap-5'>
                    {
                        inputs.map((input, index) => {
                            return (
                                <div key={index}>
                                    <h2 className='text-[#111827] text-sm'>{input.value}</h2>
                                    <input
                ...
                <button className='bg-[#111827] text-[#F1FAEE] uppercase rounded w-full mt-5 py-2 px-2' onClick={updateTimeDefaultValue}>
                    save
                </button>
            </div>
        </>
